...
/atom/movable
	...
	var/blooper_loudness = 1
	var/blooper_volume = 50
	var/blooper_volume_default  = 50 // Default volume equals to loudness 1 in blooper settings
...
/atom/movable/proc/set_blooper(id)
	if(!id)
		return FALSE
	var/datum/blooper/B = GLOB.blooper_list_ids[id]
	if(!B)
		return FALSE
...
/atom/movable/proc/blooper(list/listeners, distance, volume, pitch, queue_time)
	...
	if(!blooper)
		if(!blooper_id || !set_blooper(blooper_id)) //just-in-time blooper generation
			return
	volume = min(volume, 100) // Cap blooper volume at 100

	var/turf/T = get_turf(src)
	for(var/mob/M in listeners)
		// Volume scales with your volume slider percentage in game preferences.
		var/volumePlayAt = volume / 100 * M.client.prefs?.read_preference(/datum/preference/numeric/sound_blooper_volume)
		M.playsound_local(T, vol = volumePlayAt, vary = TRUE, frequency = pitch, max_distance = distance, falloff_distance = 0, falloff_exponent = BLOOPER_SOUND_FALLOFF_EXPONENT, sound_to_use = blooper, distance_multiplier = 1)
...
/atom/movable/send_speech(message, range = 7, obj/source = src, bubble_type, list/spans, datum/language/message_language, list/message_mods = list(), forced = FALSE, tts_message, list/tts_filter)
	...
	if(blooper || blooper_id)
		for(var/mob/M in listeners)
			if(!M.client || !M.client.prefs?.read_preference(/datum/preference/toggle/hear_sound_blooper))
				listeners -= M
...
/mob/living/carbon/human/Initialize(mapload)
	. = ..()
	// This gives a random blooper to a random created person
	if(!client)
		set_blooper(pick(GLOB.blooper_list_ids))
		...
		blooper_loudness = rand(BLOOPER_DEFAULT_MINLOUDNESS, BLOOPER_DEFAULT_MAXLOUDNESS)
...
/mob/living/send_speech(message_raw, message_range = 6, obj/source = src, bubble_type = bubble_icon, list/spans, datum/language/message_language = null, list/message_mods = list(), forced = null, tts_message, list/tts_filter)
	. = ..()
	blooper_volume = blooper_volume_default * client.prefs?.read_preference(/datum/preference/numeric/blooper_loudness) // Scale default blooper volume based on character blooper lounndness

	if(HAS_TRAIT(src, TRAIT_SIGN_LANG) && !HAS_TRAIT(src, TRAIT_MUTE)) // If you can speak and you sign, your hands don't make a bloop. Unless you are completely mute, you can have some hand bloop.
		return
	if(message_mods[WHISPER_MODE])
		blooper_volume = blooper_volume_default * 0.5 // Whispered bloops are half as loud.
		message_range++
	var/is_yell = (say_test(message_raw) == "2")
	if (is_yell)
		blooper_volume = blooper_volume_default * 2 // Yelled bloops are twice as loud.
	...
	if(blooper || blooper_id)
		...
		for(var/i in 1 to bloopers)
			if(total_delay > BLOOPER_MAX_TIME)
				break
			addtimer(CALLBACK(src, TYPE_PROC_REF(/atom/movable, blooper), listeners, message_range + 1, blooper_volume, BLOOPER_DO_VARY(blooper_pitch, blooper_pitch_range), blooper_current_blooper), total_delay) // The function is zero on the seventh tile. This makes it a maximum of 1 more.
